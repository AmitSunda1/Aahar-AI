...
function clampWithAudit(field, value, min, max, clampedInputs) {
...
function calculateDailyCalorieTarget(tdee, bmr, goal, medicalConditions = new Set(), conservativeMode = false) {
    let target;
    switch (goal) {
        case "lose_weight":
 ...
 */
function calculateMicroLimits(dailyCalorieTarget, medicalConditions = new Set()) {
    ...
    const atRisk = medicalConditions.has("hypertension") || medicalConditions.has("ckd");
...
function buildMedicalOverrides(medicalConditions, unknownMedicalConditions, conservativeModeApplied) {
    const overrides = [];
    if (medicalConditions.has("ckd")) {
        overrides.push({
            condition: "CKD",
    ...
    if (medicalConditions.has("pregnancy")) {
        overrides.push({
            condition: "Pregnancy",
            description: "Weight-loss calorie deficits are disabled and maintenance calories are used. Clinical follow-up is strongly recommended.",
        });
    }
    if (unknownMedicalConditions.length) {
        overrides.push({
            condition: "Unrecognized Condition",
            description: "One or more medical conditions are not recognized by the rules engine. Conservative targets were applied and clinician review is recommended.",
        });
    }
    if (conservativeModeApplied && !unknownMedicalConditions.length) {
        overrides.push({
            condition: "Safety Mode",
            description: "Conservative calorie targeting was applied to prioritize safety over aggressive goal progression.",
        });
    }
    return overrides;
}
...
function computeNutritionProfile(input) {
    ...
    const clampedInputs = [];
    const safeWeightKg = clampWithAudit("weightKg", weightKg, SAFE_RANGE.weightKg.min, SAFE_RANGE.weightKg.max, clampedInputs);
    const safeHeightCm = clampWithAudit("heightCm", heightCm, SAFE_RANGE.heightCm.min, SAFE_RANGE.heightCm.max, clampedInputs);
    const safeAge = clampWithAudit("age", age, SAFE_RANGE.age.min, SAFE_RANGE.age.max, clampedInputs);
    const safeDailySteps = clampWithAudit("dailySteps", dailySteps, SAFE_RANGE.dailySteps.min, SAFE_RANGE.dailySteps.max, clampedInputs);
    const safeExerciseMinutes = clampWithAudit("exerciseMinutesPerDay", exerciseMinutesPerDay, SAFE_RANGE.exerciseMinutesPerDay.min, SAFE_RANGE.exerciseMinutesPerDay.max, clampedInputs);
    const normalizedConditions = normalizeMedicalConditions(medicalConditions);
    const conservativeModeApplied = normalizedConditions.unknown.length > 0 ||
    ...
    const dailyCalorieTarget = calculateDailyCalorieTarget(maintenanceCalories, bmr, goal, normalizedConditions.canonical, conservativeModeApplied);
    const macros = calculateMacros(dailyCalorieTarget, safeWeightKg, resolvedActivityLevel, goal, normalizedConditions.canonical);
    const waterMl = calculateWaterMl(safeWeightKg, safeExerciseMinutes);
    const microLimits = calculateMicroLimits(dailyCalorieTarget, normalizedConditions.canonical);
    const medicalOverridesApplied = buildMedicalOverrides(normalizedConditions.canonical, normalizedConditions.unknown, conservativeModeApplied);
    return {
        bmr,
        maintenanceCalories,
        dailyCalorieTarget,
