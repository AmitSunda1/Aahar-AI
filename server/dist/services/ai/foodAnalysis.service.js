...
async function performGeminiAnalysis(imageBase64, mimeType, request) {
    ...
    const validated = foodAnalysis_validator_1.analyzeFoodResponseValidator.safeParse(parsed);
    if (!validated.success) {
        console.error("Validation error:", validated.error.issues);
        throw new appError_1.default(validated.error.issues[0]?.message || "Invalid Gemini food analysis response", 502, "INVALID_GEMINI_RESPONSE");
    }
 ...
 */
function buildFoodAnalysisPrompt(request) {
