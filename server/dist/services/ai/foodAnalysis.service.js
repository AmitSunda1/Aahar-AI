...
exports.analyzeFoodText = exports.analyzeFoodImage = void 0;
const generative_ai_1 = require("@google/generative-ai");
const env_config_1 = require("../../config/env.config");
const appError_1 = __importDefault(require("../../utils/appError"));
const rateLimitManager_1 = require("./rateLimitManager");
const imageOptimization_service_1 = require("./imageOptimization.service");
...
const analyzeFoodImage = async (imageBase64, mimeType, request) => {
    ...
    try {
        ...
        const response = await rateLimitManager_1.rateLimitManager.executeWithRetry(() => performGeminiAnalysis(imageBase64, mimeType, request), "Food image analysis");
        return response;
    }
    catch (error) {
 ...
 */
async function performGeminiAnalysis(imageBase64, mimeType, request) {
    // Optimize image before sending to Gemini (reduces token usage)
    const { optimizedBase64, stats } = await (0, imageOptimization_service_1.optimizeImageForGemini)(imageBase64, mimeType);
    console.log(`[Food Analysis] Image optimization: ${stats.originalSizeKb}KB → ${stats.optimizedSizeKb}KB (${stats.compressionRatio}% reduction)`);
 ...
 */
async function performGeminiTextAnalysis(request) {
    const client = new generative_ai_1.GoogleGenerativeAI(env_config_1.env.GEMINI_API_KEY);
    const model = client.getGenerativeModel({ model: env_config_1.env.GEMINI_MODEL });
    const result = await model.generateContent({
        contents: [
...
function parseFoodAnalysisResponse(rawResponse) {
    ...
    const validated = foodAnalysis_validator_1.analyzeFoodResponseValidator.safeParse(normalized);
    if (!validated.success) {
        console.error("Validation error:", validated.error.issues);
        console.error("Raw Gemini response (truncated):", rawResponse.slice(0, 500));
        throw new appError_1.default(validated.error.issues[0]?.message ||
            "Invalid Gemini food analysis response", 502, "INVALID_GEMINI_RESPONSE");
    }
 ...
 */
function buildFoodImageAnalysisPrompt(request) {
...
function buildFoodTextAnalysisPrompt(request) {
    const requestContext = buildFoodRequestContext(request);
    return `You are a nutritionist AI assistant specializing in food analysis.
...
function buildFoodRequestContext(request) {
