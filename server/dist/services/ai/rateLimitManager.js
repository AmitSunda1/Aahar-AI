...
class RateLimitManager {
    constructor() {
        ...
        this.minDelayMs = 1000; // Minimum 1 second between requests (free tier limitation)
        this.retryDelayMs = 5000; // Start with 5 second retry delay
        this.maxRetries = 3;
    }
    /**
        ...
        for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
            try {
                ...
                if (attempt === this.maxRetries) {
                    throw new Error(`${operationName} exceeded retry limit. ${lastError.message}`);
                }
                console.warn(`[RateLimitManager] ${operationName} rate limited (attempt ${attempt + 1}/${this.maxRetries}). Retrying in ${delay}ms...`);
    ...
    async processQueue() {
        ...
        while (this.queue.length > 0) {
            ...
            if (!request)
                break;
            try {
                const result = await this.executeWithRetry(() => request.execute(), `Queued request ${request.id}`);
