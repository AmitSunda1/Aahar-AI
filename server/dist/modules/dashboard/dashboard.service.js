...
exports.buildDashboardFromState = exports.mapGeminiMealPlanToUserPlan = exports.buildGeminiMealPlanPrompt = exports.buildPlanFromProfile = exports.computeUserNutritionProfile = void 0;
const nutrition_calculator_1 = require("../../utils/nutrition.calculator");
// ─── Internal helpers ─────────────────────────────────────────────────────────
const resolveUserInputs = (user) => ({
    weightKg: user.weight
        ? (0, nutrition_calculator_1.toWeightKg)(Number(user.weight.value), user.weight.unit)
        : 70,
    heightCm: user.height
        ? (0, nutrition_calculator_1.toHeightCm)(Number(user.height.value), user.height.unit)
        : 170,
});
// ─── Public: compute the full nutrition profile for a user ────────────────────
const computeUserNutritionProfile = (user) => {
    const { weightKg, heightCm } = resolveUserInputs(user);
    const activityLevel = user.activityLevel ?? "moderate";
    const resolvedActivityLevel = (0, nutrition_calculator_1.resolveActivityLevel)(activityLevel, user.dailySteps);
    return (0, nutrition_calculator_1.computeNutritionProfile)({
        weightKg,
        heightCm,
        age: user.age ?? 25,
        ...
        exerciseMinutesPerDay: (0, nutrition_calculator_1.exerciseMinutesFromActivity)(resolvedActivityLevel),
        dietaryPreferences: user.dietaryPreferences ?? [],
        medicalConditions: user.medicalConditions ?? [],
    });
};
exports.computeUserNutritionProfile = computeUserNutritionProfile;
// ─── Build a UserPlan from onboarding profile (stored in UserProgress) ────────
const buildPlanFromProfile = (user) => {
    ...
    const exerciseMinutes = (0, nutrition_calculator_1.exerciseMinutesFromActivity)(nutritionProfile.resolvedActivityLevel);
    const { dailyCalorieTarget, macros, waterMl } = nutritionProfile;
    const targetWeightKg = goal === "lose_weight"
    ...
    return {
        ...
        weight: {
            ...
            weeklyChangeKg: goal === "lose_weight"
                ? -0.5
                : goal === "gain_weight" || goal === "build_muscle"
                    ? 0.3
                    : 0,
...
const buildGeminiMealPlanPrompt = (user, nutritionProfile) => {
    ...
    if (dietaryPrefs.includes("vegan"))
        dietaryRules.push("STRICTLY vegan — NO animal products including dairy and eggs");
    ...
    if (dietaryPrefs.includes("gluten_free"))
        dietaryRules.push("Gluten-free — NO wheat or maida. Use rice, jowar, bajra, besan");
...
exports.buildGeminiMealPlanPrompt = buildGeminiMealPlanPrompt;
// ─── Map Gemini response to UserPlan (stored in UserProgress.activePlan) ──────
const mapGeminiMealPlanToUserPlan = (geminiPlan, profilePlan, promptContext, rawResponse, nutritionProfile) => {
...
const buildInsights = (plan, adherenceScore, stepCount) => {
    ...
    if (adherenceScore >= 80) {
        insights.push({
            ...
            subtitle: "You are tracking close to your daily targets. Keep the momentum through the evening.",
            tone: "positive",
        });
    }
    else if (stepCount > 0) {
        insights.push({
            ...
            subtitle: "Your logs are updating. Keep adding meals, steps, and exercise to improve adherence.",
            tone: "neutral",
        });
    }
    else {
        insights.push({
            ...
            subtitle: plan.recommendations.habits[0] ??
...
const buildStarterWorkoutSessions = (latestMealPlan) => {
    const today = new Date();
    const todayIsoDay = today.getDay() === 0 ? 7 : today.getDay();
    const weekStart = getWeekStart(today);
    const completedDayNumbers = [1, 3, 5].filter((dayNumber) => dayNumber < todayIsoDay);
    return completedDayNumbers.map((dayNumber, index) => {
        ...
        const plannedDay = latestMealPlan?.plan.days.find((day) => day.dayNumber === dayNumber);
        const dayLabel = plannedDay?.dayLabel ??
            completedAt.toLocaleDateString("en-US", { weekday: "long" });
        const workoutTitle = plannedDay?.workoutNote ??
            [
...
const buildDashboardFromState = (user, progress, latestMealPlan) => {
    ...
    return {
        ...
        caloriesLeft: dailyCalorieTarget - actuals.caloriesConsumed + actuals.caloriesBurned,
        macros: {
        ...
        workoutSessions: progress?.workoutSessions?.length
